...
int updateAccount(int option, char * newData, const int accountNumber){
	...
	case TRANSACTION:{
		char * endptr;
		float number = strtof(newData, &endptr);
		sprintf(selectQuery, "UPDATE RECORDS set AMOUNT = %f WHERE ACCOUNT = %d;",number, accountNumber);
		break;
		}
	}
...
int getData(struct Customer * customer, int accountNumber){
	...
	//Retrieve data from the current row
	strcpy(customer->name, (char *)sqlite3_column_text(stmt, 1));
	customer->acc_no = sqlite3_column_int(stmt, 2);
